    ...
    html2canvas(document.body, { scale: 2 }).then(canvas => {
        canvas.toBlob(async blob => {
            ...
            if (!res.ok) {
                ...
                showScreenshotNotification(); // 🔔 yangi qo'shilgan funksiya
            }
        }, 'image/png');
    });
}
...
function createMiniWindow() {
    const miniWindowHTML = `
        ...
        <div id="screenshot-notification" style="display: none;">Скриншот yuborildi</div>
    `;
...
#screenshot-notification {
    ...
    right: 2px;
    font-size: 10px;
    color: rgba(204, 204, 204, 0.75); /* Bot xabar rangi bilan bir xil */
    background: rgba(255, 255, 255, 0); /* 100% shaffof */
    z-index: 2000;
}
    `;
...
// 🔔 Скриншот yuborildi oynasi
function showScreenshotNotification() {
    const notif = document.getElementById('screenshot-notification');
    if (!notif) return;
